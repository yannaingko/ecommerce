...
class CartComponent extends Component
{
    ...
    {
        $cpcode = Coupon::where('code',$this->coupon)->where('quantities','>',0)->where('amount_limit','<=',Cart::instance('cart')->subtotal())->first();
        if(!$cpcode)
        {
    ...
    public function setAmountForCheckout()
    {
        ...
        if(session()->has('coupon'))
        {
            ...
            session()->put('checkout',[
                ...
                'total' => Cart::instance('cart')->total()+ $deliveryCost,
                'coupon'  => $this->coupon,
            ]);
        }
    }
